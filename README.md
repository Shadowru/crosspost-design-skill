...
scripts/
  ...
  make_web_zip.py               архив для загрузки в claude.ai
...
Веб-версия ставит скиллы не из маркетплейса, а загрузкой архива, и требует
включённого выполнения кода — без него скрипты скилла работать не смогут.
...
crosspost-design, чтобы…».

> [!NOTE]
> Что в вебе работает не так. В архив не попадают картинки README, eval-кейсы и
> манифесты плагина — в песочнице от них нет пользы. `telegraph_publish.py`
> ходит в `api.telegra.ph`, и если в вашей песочнице нет исходящей сети,
> публиковать страницу придётся из CLI или руками. `wrap_preview.py` соберёт
> страницу с кнопкой «Копировать», но открывать её нужно у себя, скачав файл.
